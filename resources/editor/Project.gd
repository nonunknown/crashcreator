...
var enemy_ids:Array = []
var enemy_pos:PoolVector3Array = []
...
func append_enemy_pos(pos:Vector3):
...
func to_dict()-> Dictionary:
	var dict:Dictionary = {crate_pos={xs=[],ys=[],zs=[]},
						   entity_pos={xs=[],ys=[],zs=[]},
						   enemy_pos={xs=[],ys=[],zs=[]}
						}
	...
	dict["enemy_ids"] = enemy_ids
	to_pool_vector3_dict(enemy_pos,dict,"enemy_pos")
	return dict
...
func from_dict(dict:Dictionary) -> void:
	...
	if dict.has("enemy_ids"):
		enemy_ids = dict.enemy_ids
		enemy_pos = from_pool_vector3_dict(dict,"enemy_pos")
