...
export var path_tab_properties:NodePath
export var path_tab_data:NodePath
var tab_properties:Node
var tab_data:Node
...
func _ready() -> void:
	tab_properties = get_node(path_tab_properties)
	tab_data = get_node(path_tab_data)
	tab_properties.get_node("new_property_btn").connect("pressed", self, "on_new_property_btn_pressed")
...
func set_table(table : Object) -> void:
	...
	for idx in range(0, table.get_props_count()):
		...
		tab_data.get_node("data_holder/data_header").add_child(lbl)
	...
	for idx in range(0, table.get_rows_count()):
		...
		tab_data.get_node("data_holder/data_container").add_child(row)
...
func clear_structure() -> void:
	for idx in range(0, tab_properties.get_node("properties").get_child_count()):
		var prop = tab_properties.get_node("properties").get_child(idx)
...
func on_new_property_btn_pressed() -> void:
	...
	m_props.push_back(prop)
	tab_properties.get_node("properties").add_child(prop)
	...
	var rows = tab_data.get_node("data_holder/data_container").get_child_count()
	for idx in range(0, rows-1):
		var row = tab_data.get_node("data_holder/data_container").get_child(idx)
		var cell = load("res://addons/godot_db_manager/table_cell.tscn").instance()
		cell.set_text("")
...
func on_edit_property(prop_id : int, prop_type : int, prop_name : String) -> void:
	# update data header
	for idx in range(0, tab_data.get_node("data_holder/data_header").get_child_count()):
		if(tab_data.get_node("data_holder/data_header").get_child(idx).get_prop_id() == prop_id):
			tab_data.get_node("data_holder/data_header").get_child(idx).set_text(prop_name)
...
func on_delete_property(prop_id : int) -> void:
	for idx in range(0, m_props.size()):
		if(m_props[idx].get_prop_id() == prop_id):
			...
			tab_properties.get_node("properties").remove_child(m_props[idx])
			m_props.remove(idx)
			break
